...
const app = express();
// In a production environment, the port would typically be read from an environment variable.
const port = 5000;
...
app.post('/bfhl', (req, res) => {
  ...
  let concat_string = "";
  const reversedAlphabets = [...alphabets].map(a => a.toLowerCase()).reverse();
  ...
  const response = {
    is_success: true,
    // In a real system, this data would be dynamic (e.g., fetched from a database or authentication service).
    user_id: "ankit_goel",
    // In a real system, this data would be dynamic (e.g., fetched from a database or authentication service).
